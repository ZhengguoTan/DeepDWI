...
parser.add_argument('--data_dir', default='2024-05-13_zsssl_0.7mm_21-dir_R2x2_vol2_scan2_kdat_slice_040_norm-kdat-1.0_navi_ResNet2D_ResBlock-12_kernel-3_ADMM_08_lamda-0.050_Adam_lr-0.000500_MixL1L2Loss',
...
parser.add_argument('--flip_axes', nargs='+', type=int,
                    default=None)

args = parser.parse_args()

if args.slice_idx >= 0:
    slice_loop = range(args.slice_idx, args.slice_idx + 1, 1)
else:
    slice_loop = range(args.slices)

# %%
DATA_DIR = DIR + '/' + args.data_dir
...
for s in slice_loop:
    ...
    fstr = DATA_DIR + '/' + args.method_pre + '_slice_' + str(s).zfill(3) + args.method_post
...
if args.zf > N_y and args.zf > N_x:
    ...
    print('- zf --> ', reo_slice.shape)

if args.flip_axes is not None:
    print('- flip axes: ', args.flip_axes)
    reo_slice = np.flip(reo_slice, axis=tuple(args.flip_axes))

print('> reo_slice shape: ', reo_slice.shape)

f = h5py.File(DATA_DIR + '/' + args.method_pre + args.method_post + '.h5', 'w')
