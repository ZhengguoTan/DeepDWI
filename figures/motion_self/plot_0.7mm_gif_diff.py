...
# %% axial
vmax_jets = np.amax(abs(DWI_JETS_SELF)) * 0.04
...
for d in range(1, N_diff, 1):
    ...
    # self-gating
    ax[0].imshow(abs(np.flip(DWI_JETS_SELF[d, tra_slice_idx, :, :], axis=-2)),
                    cmap='gray', interpolation=None,
                    vmin=0, vmax=vmax_jets)

    ax[1].imshow(abs(np.flip(DWI_JETS_SELF[d, (N_z - int(N_x/2)):, cor_slice_idx, :], axis=-2)),
                    cmap='gray', interpolation=None,
                    vmin=0, vmax=vmax_jets)

    ax[2].imshow(abs(np.flip(DWI_JETS_SELF[d, (N_z - int(N_x/2)):, :, sag_slice_idx], axis=-2)),
                    cmap='gray', interpolation=None,
                    vmin=0, vmax=vmax_jets)
    ...
    ax[0].text(0.03*N_x, 0.08*N_y, 'Self-Gated LLR', bbox=props,
